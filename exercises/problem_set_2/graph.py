...
# Collaborators: Fernando Corcuera & Diego the cat
# Time:
...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        return (
            f"{self.src}->{self.dest} ({self.total_distance}, {self.outdoor_distance})"
        )
...
class Digraph:
    ...
    def add_node(self, node):
        ...
        if node in self.nodes or node in self.edges.keys():
    ...
    def add_edge(self, edge):
        ...
        if not (src in self.nodes and dst in self.nodes):
            raise ValueError("Source and destination not in graph")

        weighted_dst = WeightedEdge(
            src, dst, edge.total_distance, edge.outdoor_distance
        )
        self.edges[src].append(weighted_dst)
