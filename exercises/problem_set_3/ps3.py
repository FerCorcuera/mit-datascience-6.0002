...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for w in range(self.width):
            for h in range(self.height):
                tile = (w, h)
                self.tiles[tile] = self.dirt_amount
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        new_dirt = self.tiles[tile] - capacity

        if new_dirt < 0:
            self.tiles[tile] = 0

        else:
            self.tiles[tile] = new_dirt
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        """
        original_n_tiles = len(self.tiles)
        fornitures = len(self.furniture_tiles)
        return -fornitures + original_n_tiles
    ...
    def get_random_position(self):
        ...
        is_valid = self.is_position_valid(Position(x, y))
...
def show_plot_compare_strategies(title, x_label, y_label):
    ...
    for num_robots in num_robot_range:
        ...
        times1.append(
            run_simulation(num_robots, 1.0, 1, 20, 20, 3, 0.8, 20, StandardRobot)
        )
        times2.append(
            run_simulation(num_robots, 1.0, 1, 20, 20, 3, 0.8, 20, FaultyRobot)
        )
...
def show_plot_room_shape(title, x_label, y_label):
    ...
    for width in [10, 20, 25, 50]:
        # Room dimensions are tile counts and must stay integers on Python 3.
        height = 300 // width
        print(
